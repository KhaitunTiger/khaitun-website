    ...
    <nav className={`fixed w-full z-40 transition-all ${scrolled ? 'bg-white/90 dark:bg-black/90 backdrop-blur-sm shadow-lg' : 'bg-white dark:bg-black'}`}>
      <div className="container mx-auto px-4">
        <div className="flex flex-wrap items-center justify-between">
          ...
          {/* Mobile menu */}
          <div className={`w-full lg:hidden ${isMenuOpen ? 'block' : 'hidden'} bg-white dark:bg-black border-t dark:border-gray-800`}>
            <div className="flex flex-col py-4 px-4 space-y-4">
              <div className="flex items-center justify-center gap-6 py-4 border-b dark:border-gray-800">
                  <a href="https://x.com/KhaitunTiger" target="_blank" rel="noopener noreferrer" className="hover:opacity-80 transition-opacity">
              ...
              <select
                value={language}
                onChange={(e) => setLanguage(e.target.value as 'en' | 'zh' | 'th')}
                className="bg-transparent border border-gray-300 rounded px-3 py-1.5 text-base focus:outline-none focus:border-blue-500 dark:border-gray-700"
              >
                <option value="en">English</option>
                <option value="zh">中文</option>
                <option value="th">ไทย</option>
              </select>
              <a href="#hero" onClick={() => setIsMenuOpen(false)} className="block py-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Home</a>
              <a href="#about" onClick={() => setIsMenuOpen(false)} className="block py-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">About</a>
              <a href="#store" onClick={() => setIsMenuOpen(false)} className="block py-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Store</a>
              <a href="#whitepaper" onClick={() => setIsMenuOpen(false)} className="block py-2 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Whitepaper</a>
              
              {/* Mobile wallet section */}
              {!isConnected ? (
                <div className="flex flex-col items-center space-y-4 mt-8 border-t dark:border-gray-800 pt-4">
                  ...
                  {error && (
                    <div className="text-sm text-red-500 text-center max-w-[280px]">{error}</div>
                  )}
                </div>
              ) : (
